...
public sealed class LauncherAuthHelper(IHttpContextAccessor accessor, IOptions<LauncherOptions> options)
{
    public bool IsAuthValid([NotNullWhen(false)] out IActionResult? failureResult)
    {
        ...
        {
            failureResult = new ObjectResult("Launcher.PublishToken is not set")
    ...
    { return CryptographicOperations.FixedTimeEquals(MemoryMarshal.AsBytes(a), MemoryMarshal.AsBytes(b)); }
}
